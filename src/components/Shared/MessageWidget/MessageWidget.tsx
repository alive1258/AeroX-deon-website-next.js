...
const STAFF_FALLBACK_NAME = "AeroX Support";
...
const MessageWidget = ({
  ...
  return (
    <>
      {/* PANEL */}
      <div
        className={`fixed bottom-24 right-4 z-90 flex w-[calc(100vw-2rem)] max-w-sm origin-bottom-right flex-col overflow-hidden rounded-2xl border border-white/10 bg-ink-900 shadow-2xl transition duration-200 sm:right-6 ${
          isOpen
      ...
      >
        <div className="flex items-start justify-between gap-3 bg-ink-950 p-5">
          <div>
            <p className="font-bold text-white">Chat with AeroX</p>
        ...
        {!isLoggedIn ? (
          <div className="flex flex-col items-center gap-2.5 px-5 py-10 text-center">
            ...
            <p className="text-sm font-bold text-white">Log In to Chat</p>
            ...
            <button
              type="button"
              onClick={goToLogin}
              className="mt-2 rounded-lg bg-orange-500 px-4 py-2 text-xs font-semibold text-white transition hover:bg-orange-600"
            >
              Log In / Sign Up
            </button>
          </div>
          ...
          <>
            ...
            >
              {isHistoryLoading ? (
                <div className="space-y-2.5">
                  {[1, 2, 3].map((i) => (
                    <div
                      key={i}
                      className="h-10 w-2/3 animate-pulse rounded-2xl bg-white/10"
                    />
                  ))}
                </div>
              ) : messages.length === 0 ? (
                    ...
                    >
                      {!isMine &&
                        (isGrouped ? (
                          ...
                          <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-ink-900 text-[11px] font-bold text-white">
                            {initialOf(name!)}
                          </div>
                        ))}
                      ...
                      >
                        {!isMine && !isGrouped && (
                          <span className="mb-0.5 ml-1 text-[11px] font-semibold text-white/50">
                            {name}
                          </span>
                        )}
                        <div
              ...
              {otherPartyTyping && (
                <div className="mt-3 flex items-end justify-start gap-2">
                  ...
                  <div className="rounded-2xl rounded-bl-sm bg-ink-900 px-3.5 py-2.5 shadow-sm">
            ...
            >
              <input
                ...
                className="flex-1 rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-white placeholder:text-white/30 focus:border-orange-500 focus:outline-none"
              />
              <button
                type="submit"
                disabled={!draft.trim()}
      ...
      <button
        ...
        className="fixed bottom-6 right-6 z-90 hidden h-14 w-14 cursor-pointer items-center justify-center rounded-full bg-orange-500 text-white shadow-xl transition hover:scale-105 hover:bg-orange-600 lg:flex"
      >
        {isOpen ? (
