  ...
  return (
    <>
      <header className="fixed top-0 w-full z-50">
        {/* TOP UTILITY BAR */}
        <div className="hidden lg:block bg-ink-950 text-white/80">
          <div className="container flex items-center justify-between h-9 text-xs">
            ...
            <Link
              href="/shop"
              className="bg-orange-500 text-white px-3.5 py-1 rounded-full text-xs font-semibold hover:bg-orange-600 transition"
            >
              Preorder Now
            </Link>
          </div>
        ...
        {/* MAIN NAVBAR */}
        <div
          className={`md:bg-transparent transition lg:border-b lg:border-white/10 lg:bg-ink-900 ${
              ...
              {MENU_ITEMS.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <div className="overflow-hidden rounded-xl border border-white/10 bg-ink-950 py-2 shadow-xl">
                        {item.children.map((child) => (
                          <Link
                            ...
                            onClick={() => setOpenDropdown(null)}
                            onFocus={() => setOpenDropdown(item.href)}
                            className="block px-4 py-2.5 text-sm text-white/70 transition hover:bg-white/5 hover:text-orange-400"
                          >
                            {child.display}
                          </Link>
                        ))}
      ...
      {/* spacer — reserved only at lg+, where the navbar is solid ink-900;
