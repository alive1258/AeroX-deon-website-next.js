...
} from "@/src/redux/api/accessoriesBannerApi";

interface EditAccessoriesBannerProps {
  id: string;
}

interface EditAccessoriesBannerFormValues {
  title: string;
  eyebrow?: string;
  button_text?: string;
  button_link?: string;
...
const ALL_ACCESSORIES_BANNER_PATH = "/dashboard/accessories-banner/all-accessories-banner";

const EditAccessoriesBanner = ({ id }: EditAccessoriesBannerProps) => {
  const router = useRouter();

  const [imagePreview, setImagePreview] = useState<string | null>(null);

  const { data: bannerData, isLoading: isFetching } = useGetSingleAccessoriesBannerQuery(id);
  const [updateAccessoriesBanner, { isLoading: isUpdating }] = useUpdateAccessoriesBannerMutation();
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<EditAccessoriesBannerFormValues>();
  ...
  const onSubmit: SubmitHandler<EditAccessoriesBannerFormValues> = async (values) => {
    try {
      ...
      await updateAccessoriesBanner({ id, data: formData }).unwrap();
      toast.success("Accessories banner updated successfully!");
      router.push(ALL_ACCESSORIES_BANNER_PATH);
    } catch (err) {
      ...
      Swal.fire({
        ...
        text:
          (Array.isArray(error.data?.message)
            ...
            : error.data?.message) || "Failed to update accessories banner.",
        icon: "error",
      });
    }
  };
    ...
    return (
      <div className="flex h-64 items-center justify-center rounded-lg border bg-white p-6">
        <div className="flex items-center gap-2 text-gray-500">
          ...
          <span>Loading accessories banner details...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-lg border bg-white border-gray-200 overflow-hidden p-6">
      <PageHeader
        title="Edit Accessories Banner"
        breadcrumbs={[
          { title: "Dashboard", link: "/dashboard" },
          { title: "Accessories Banner", link: ALL_ACCESSORIES_BANNER_PATH },
          { title: "Edit Accessories Banner" },
        ]}
      />
...
export default EditAccessoriesBanner;
