...
import { AccessoriesBannerItem } from "@/src/types/accessoriesBannerType";
...
const AllAccessoriesBanner: React.FC = () => {
  const [searchValue, setSearchValue] = useState("");
  const [currentPage, setCurrentPage] = useState(1);

  const debouncedSearch = useDebounce(searchValue, 500);

  const { data, isLoading, isFetching, refetch } = useGetAllAccessoriesBannerQuery({
    search: (debouncedSearch as string) || undefined,
    page: currentPage,
    limit: LIMIT,
  });

  const [deleteAccessoriesBanner] = useDeleteAccessoriesBannerMutation();

  const banners: AccessoriesBannerItem[] = data?.data || [];
  ...
  const handleDeleteBanner = async (banner: AccessoriesBannerItem) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Delete accessories banner "${banner.title}"?`,
        icon: "warning",
      ...
      await deleteAccessoriesBanner(banner.id).unwrap();

      await Swal.fire({
        icon: "success",
        title: "Deleted!",
        text: `Accessories banner "${banner.title}" has been deleted.`,
        timer: 1000,
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 p-6 border-b border-gray-200">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Accessories Banner</h1>
          <p className="text-sm text-gray-500">
            Manage the homepage accessories banner content
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
          ...
          <Link href="/dashboard/accessories-banner/add-accessories-banner">
            ...
            {banners?.length > 0 ? (
                ...
                >
                  ...
                  <td className="px-5 py-3">
                    <div className="flex justify-center gap-2">
                      <Link href={`/dashboard/accessories-banner/edit-accessories-banner/${banner.id}`}>
                        <button
              ...
              <tr>
                <td colSpan={8} className="py-10 text-center text-gray-500">
                  No accessories banner entries found.
                </td>
              </tr>
            )}
          </tbody>
...
export default AllAccessoriesBanner;
