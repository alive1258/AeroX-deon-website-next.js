...
import { useCreateAccessoriesBannerMutation } from "@/src/redux/api/accessoriesBannerApi";

interface AddAccessoriesBannerFormValues {
  title: string;
  eyebrow?: string;
  button_text?: string;
  button_link?: string;
...
const AddAccessoriesBanner = () => {
  const router = useRouter();
  const [imagePreview, setImagePreview] = useState<string | null>(null);

  const [createAccessoriesBanner, { isLoading }] = useCreateAccessoriesBannerMutation();
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<AddAccessoriesBannerFormValues>({
    defaultValues: {
      title: "",
  ...
  const onSubmit: SubmitHandler<AddAccessoriesBannerFormValues> = async (values) => {
    try {
      ...
      await createAccessoriesBanner(formData).unwrap();
      toast.success("Accessories banner created successfully!");
      reset();
      setImagePreview(null);
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-8 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Input
            label="Title"
            text="title"
            placeholder="Accessories That Elevate Every Flight"
        ...
        <div className="flex justify-end gap-4 mt-6">
          ...
          <GradientButton
            type="submit"
            text={isLoading ? "Saving..." : "Create Banner"}
            icon={Plus}
            disabled={isLoading}
          />
        </div>
...
export default AddAccessoriesBanner;
