...
import { FlightModeItem } from "@/src/types/flightModeType";
import { ApiError } from "@/src/types/authType";
import Pagination from "@/src/utils/Pagination";
import {
  useDeleteFlightModeMutation,
  useGetAllFlightModeQuery,
} from "@/src/redux/api/flightModeApi";

const LIMIT = 10;

const AllFlightMode: React.FC = () => {
  const [searchValue, setSearchValue] = useState("");
  const [currentPage, setCurrentPage] = useState(1);

  const debouncedSearch = useDebounce(searchValue, 500);

  const { data, isLoading, isFetching, refetch } = useGetAllFlightModeQuery({
    search: (debouncedSearch as string) || undefined,
    page: currentPage,
    limit: LIMIT,
  });

  const [deleteFlightMode] = useDeleteFlightModeMutation();

  const flightModes: FlightModeItem[] = data?.data || [];
  ...
  const handleDeleteFlightMode = async (flightMode: FlightModeItem) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Delete flight mode "${flightMode.title}"?`,
        icon: "warning",
      ...
      await deleteFlightMode(flightMode.id).unwrap();

      await Swal.fire({
        icon: "success",
        title: "Deleted!",
        text: `Flight mode "${flightMode.title}" has been deleted.`,
        timer: 1000,
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 p-6 border-b border-gray-200">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Flight Modes</h1>
          <p className="text-sm text-gray-500">
            Manage the homepage flight modes cards
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
          <input
            type="text"
            placeholder="Search flight modes..."
            value={searchValue}
          ...
          <Link href="/dashboard/flight-modes/add-flight-mode">
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full">
          ...
          <tbody>
            {flightModes?.length > 0 ? (
              flightModes.map((flightMode, index) => (
                <tr
                  key={flightMode.id}
                ...
                >
                  ...
                  <td className="px-5 py-3">
                    {flightMode.image ? (
                      <Image
                        src={flightMode?.image}
                        alt={flightMode?.title}
                        width={40}
                        height={40}
                  ...
                  <td className="px-5 py-3 text-center">
                    ...
                    >
                      {flightMode.featured ? "Featured" : "Standard"}
                    </span>
                  </td>
                  ...
                  <td className="px-5 py-3 text-center">
                    <span
                      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${
                        flightMode.is_active
                          ? "bg-emerald-50 text-emerald-700 border-emerald-200"
                          : "bg-gray-50 text-gray-500 border-gray-200"
                      }`}
                    >
                      {flightMode.is_active ? "Active" : "Inactive"}
                    </span>
                  </td>
                  ...
                  <td className="px-5 py-3">
                    <div className="flex justify-center gap-2">
                      <Link
                        href={`/dashboard/flight-modes/edit-flight-mode/${flightMode.id}`}
                      >
                        <button
                      ...
                      <button
                        onClick={() => handleDeleteFlightMode(flightMode)}
