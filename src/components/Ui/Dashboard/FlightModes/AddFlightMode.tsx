...
import { useCreateFlightModeMutation } from "@/src/redux/api/flightModeApi";

interface AddFlightModeFormValues {
  title: string;
  description?: string;
  featured: boolean;
  position?: number;
...
const AddFlightMode = () => {
  ...
  const [createFlightMode, { isLoading }] = useCreateFlightModeMutation();
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<AddFlightModeFormValues>({
    defaultValues: {
      title: "",
      featured: false,
      position: 1,
  ...
  const onSubmit: SubmitHandler<AddFlightModeFormValues> = async (values) => {
    try {
      ...
      await createFlightMode(formData).unwrap();
      toast.success("Flight mode created successfully!");
      reset();
      setImagePreview(null);
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-8 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <Textarea
            ...
            text="description"
            placeholder="Your drone. Your shadow."
        ...
        <div className="flex justify-end gap-4 mt-6">
          ...
          <GradientButton
            type="submit"
            text={isLoading ? "Saving..." : "Create Flight Mode"}
            icon={Plus}
            disabled={isLoading}
          />
        </div>
