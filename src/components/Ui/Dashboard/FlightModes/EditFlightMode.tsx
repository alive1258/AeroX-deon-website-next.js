...
interface EditFlightModeProps {
  id: string;
}

interface EditFlightModeFormValues {
  title: string;
  description?: string;
  featured: boolean;
  position?: number;
...
const ALL_FLIGHT_MODES_PATH = "/dashboard/flight-modes/all-flight-modes";

const EditFlightMode = ({ id }: EditFlightModeProps) => {
  const router = useRouter();

  const [imagePreview, setImagePreview] = useState<string | null>(null);

  const { data: flightModeData, isLoading: isFetching } =
    useGetSingleFlightModeQuery(id);
  const [updateFlightMode, { isLoading: isUpdating }] =
    useUpdateFlightModeMutation();
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<EditFlightModeFormValues>();
  ...
  useEffect(() => {
    if (flightModeData?.data) {
      const item = flightModeData.data;
      reset({
        title: item.title || "",
  ...
  }, [flightModeData, reset]);

  const onSubmit: SubmitHandler<EditFlightModeFormValues> = async (values) => {
    try {
      ...
      await updateFlightMode({ id, data: formData }).unwrap();
      toast.success("Flight mode updated successfully!");
      router.push(ALL_FLIGHT_MODES_PATH);
    } catch (err) {
      ...
      Swal.fire({
        ...
        text:
          (Array.isArray(error.data?.message)
            ...
            : error.data?.message) || "Failed to update flight mode.",
        icon: "error",
      });
    }
  };
    ...
    return (
      <div className="flex h-64 items-center justify-center rounded-lg border bg-white p-6">
        <div className="flex items-center gap-2 text-gray-500">
          ...
          <span>Loading flight mode details...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-lg border bg-white border-gray-200 overflow-hidden p-6">
      <PageHeader
        title="Edit Flight Mode"
        breadcrumbs={[
          { title: "Dashboard", link: "/dashboard" },
          { title: "Flight Modes", link: ALL_FLIGHT_MODES_PATH },
          { title: "Edit Flight Mode" },
        ]}
      />
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-8 space-y-6">
        ...
        <div className="flex justify-end gap-4 mt-6">
          ...
          <GradientButton
            type="submit"
            text={isUpdating ? "Updating..." : "Update Flight Mode"}
            icon={Save}
            disabled={isUpdating}
          />
        </div>
