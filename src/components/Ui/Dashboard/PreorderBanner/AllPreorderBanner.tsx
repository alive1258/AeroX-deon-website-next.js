...
import { PreorderBannerItem } from "@/src/types/preorderBannerType";
...
const AllPreorderBanner: React.FC = () => {
  const [searchValue, setSearchValue] = useState("");
  const [currentPage, setCurrentPage] = useState(1);

  const debouncedSearch = useDebounce(searchValue, 500);

  const { data, isLoading, isFetching, refetch } = useGetAllPreorderBannerQuery({
    search: (debouncedSearch as string) || undefined,
    page: currentPage,
    limit: LIMIT,
  });

  const [deletePreorderBanner] = useDeletePreorderBannerMutation();

  const banners: PreorderBannerItem[] = data?.data || [];
  ...
  const handleDeletePreorderBanner = async (banner: PreorderBannerItem) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Delete preorder banner "${banner.title}"?`,
        icon: "warning",
      ...
      await deletePreorderBanner(banner.id).unwrap();

      await Swal.fire({
        icon: "success",
        title: "Deleted!",
        text: `Preorder banner "${banner.title}" has been deleted.`,
        timer: 1000,
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 p-6 border-b border-gray-200">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Preorder Banner</h1>
          <p className="text-sm text-gray-500">
            Manage the homepage preorder banner content
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
          <input
            type="text"
            placeholder="Search preorder banners..."
            value={searchValue}
          ...
          <Link href="/dashboard/preorder-banner/add-preorder-banner">
            ...
            {banners?.length > 0 ? (
                ...
                >
                  ...
                  <td className="px-5 py-3">
                    <div className="flex justify-center gap-2">
                      <Link
                        href={`/dashboard/preorder-banner/edit-preorder-banner/${banner.id}`}
                      >
                        <button
                      ...
                      <button
                        onClick={() => handleDeletePreorderBanner(banner)}
              ...
              <tr>
                <td colSpan={7} className="py-10 text-center text-gray-500">
                  No preorder banner entries found.
                </td>
              </tr>
            )}
          </tbody>
...
export default AllPreorderBanner;
