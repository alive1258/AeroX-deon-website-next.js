...
import { useCreatePreorderBannerMutation } from "@/src/redux/api/preorderBannerApi";

interface AddPreorderBannerFormValues {
  title: string;
  badge_text?: string;
  description?: string;
  button_text?: string;
...
const AddPreorderBanner = () => {
  ...
  const [createPreorderBanner, { isLoading }] = useCreatePreorderBannerMutation();
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<AddPreorderBannerFormValues>({
    defaultValues: {
      title: "",
  ...
  const onSubmit: SubmitHandler<AddPreorderBannerFormValues> = async (values) => {
    try {
      await createPreorderBanner({
        title: values.title,
      ...
      toast.success("Preorder banner created successfully!");
      reset();
      setBundleItems([]);
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-8 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <Input
            label="Title"
            text="title"
            placeholder="Preorder Now & Get Exclusive Accessories"
          ...
          <Textarea
            ...
            text="description"
            placeholder="Be the first to fly smarter."
        ...
        <div className="flex justify-end gap-4 mt-6">
          ...
          <GradientButton
            type="submit"
            text={isLoading ? "Saving..." : "Create Preorder Banner"}
            icon={Plus}
            disabled={isLoading}
          />
        </div>
...
export default AddPreorderBanner;
