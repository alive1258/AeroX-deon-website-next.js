...
import { ShopAccessoryItem } from "@/src/types/shopAccessoryType";
...
const AllShopAccessory: React.FC = () => {
  const [searchValue, setSearchValue] = useState("");
  const [currentPage, setCurrentPage] = useState(1);

  const debouncedSearch = useDebounce(searchValue, 500);

  const { data, isLoading, isFetching, refetch } = useGetAllShopAccessoryQuery({
    search: (debouncedSearch as string) || undefined,
    page: currentPage,
    limit: LIMIT,
  });

  const [deleteShopAccessory] = useDeleteShopAccessoryMutation();

  const accessories: ShopAccessoryItem[] = data?.data || [];
  ...
  const handleDeleteAccessory = async (accessory: ShopAccessoryItem) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Delete shop accessory "${accessory.name}"?`,
        icon: "warning",
      ...
      await deleteShopAccessory(accessory.id).unwrap();

      await Swal.fire({
        icon: "success",
        title: "Deleted!",
        text: `Shop accessory "${accessory.name}" has been deleted.`,
        timer: 1000,
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 p-6 border-b border-gray-200">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Shop Accessories</h1>
          <p className="text-sm text-gray-500">
            Manage the shop page's add-on accessory list
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
          ...
          <Link href="/dashboard/shop-accessories/add-shop-accessory">
                  ...
                  >
                    ...
                    <td className="px-5 py-3">
                      <div className="flex justify-center gap-2">
                        <Link href={`/dashboard/shop-accessories/edit-shop-accessory/${accessory.id}`}>
                          <button
...
export default AllShopAccessory;
