...
import { VideoShowcaseItem } from "@/src/types/videoShowcaseType";
...
const AllVideoShowcase: React.FC = () => {
  const [searchValue, setSearchValue] = useState("");
  const [currentPage, setCurrentPage] = useState(1);

  const debouncedSearch = useDebounce(searchValue, 500);

  const { data, isLoading, isFetching, refetch } = useGetAllVideoShowcaseQuery({
    search: (debouncedSearch as string) || undefined,
    page: currentPage,
    limit: LIMIT,
  });

  const [deleteVideoShowcase] = useDeleteVideoShowcaseMutation();

  const items: VideoShowcaseItem[] = data?.data || [];
  ...
  const handleDelete = async (item: VideoShowcaseItem) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Delete video showcase "${item.title}"?`,
        icon: "warning",
      ...
      await deleteVideoShowcase(item.id).unwrap();

      await Swal.fire({
        icon: "success",
        title: "Deleted!",
        text: `Video showcase "${item.title}" has been deleted.`,
        timer: 1000,
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 p-6 border-b border-gray-200">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Video Showcase</h1>
          <p className="text-sm text-gray-500">
            Manage the homepage "Shot on AeroX Max Pro" video reel
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
          <input
            type="text"
            placeholder="Search video showcase entries..."
            value={searchValue}
          ...
          <Link href="/dashboard/video-showcase/add-video-showcase">
            ...
            {items?.length > 0 ? (
                ...
                >
                  ...
                  <td className="px-5 py-3">
                    <div className="flex justify-center gap-2">
                      <Link href={`/dashboard/video-showcase/edit-video-showcase/${item.id}`}>
                        <button
              ...
              <tr>
                <td colSpan={8} className="py-10 text-center text-gray-500">
                  No video showcase entries found.
                </td>
              </tr>
            )}
          </tbody>
...
export default AllVideoShowcase;
