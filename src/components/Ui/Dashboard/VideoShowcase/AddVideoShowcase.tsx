...
import { useCreateVideoShowcaseMutation } from "@/src/redux/api/videoShowcaseApi";

interface AddVideoShowcaseFormValues {
  title: string;
  eyebrow?: string;
  video_url?: string;
  position?: number;
...
const AddVideoShowcase = () => {
  ...
  const [createVideoShowcase, { isLoading }] = useCreateVideoShowcaseMutation();
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<AddVideoShowcaseFormValues>({
    defaultValues: {
      title: "",
      position: 1,
      is_active: true,
  ...
  const onSubmit: SubmitHandler<AddVideoShowcaseFormValues> = async (values) => {
    try {
      ...
      await createVideoShowcase(formData).unwrap();
      toast.success("Video showcase created successfully!");
      reset();
      setImagePreview(null);
      setThumbnails([]);
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-8 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <Input
            label="Title"
            text="title"
            placeholder="Shot on AeroX Max Pro"
        ...
        <div className="flex justify-end gap-4 mt-6">
          ...
          <GradientButton
            type="submit"
            text={isLoading ? "Saving..." : "Create Video Showcase"}
            icon={Plus}
            disabled={isLoading}
          />
        </div>
...
export default AddVideoShowcase;
