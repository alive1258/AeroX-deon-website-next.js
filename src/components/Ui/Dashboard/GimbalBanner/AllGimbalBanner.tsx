...
import { GimbalBannerItem } from "@/src/types/gimbalBannerType";
import { ApiError } from "@/src/types/authType";
import Pagination from "@/src/utils/Pagination";
import {
  useDeleteGimbalBannerMutation,
  useGetAllGimbalBannerQuery,
} from "@/src/redux/api/gimbalBannerApi";

const LIMIT = 10;

const AllGimbalBanner: React.FC = () => {
  const [searchValue, setSearchValue] = useState("");
  const [currentPage, setCurrentPage] = useState(1);

  const debouncedSearch = useDebounce(searchValue, 500);

  const { data, isLoading, isFetching, refetch } = useGetAllGimbalBannerQuery({
    search: (debouncedSearch as string) || undefined,
    page: currentPage,
    limit: LIMIT,
  });

  const [deleteGimbalBanner] = useDeleteGimbalBannerMutation();

  const gimbalBanners: GimbalBannerItem[] = data?.data || [];
  ...
  const handleDeleteGimbalBanner = async (item: GimbalBannerItem) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Delete gimbal banner "${item.title}"?`,
        icon: "warning",
      ...
      await deleteGimbalBanner(item.id).unwrap();

      await Swal.fire({
        icon: "success",
        title: "Deleted!",
        text: `Gimbal banner "${item.title}" has been deleted.`,
        timer: 1000,
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 p-6 border-b border-gray-200">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Gimbal Banner</h1>
          <p className="text-sm text-gray-500">
            Manage the homepage gimbal precision-engineering banner
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
          <input
            type="text"
            placeholder="Search gimbal banners..."
            value={searchValue}
          ...
          <Link href="/dashboard/gimbal-banner/add-gimbal-banner">
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full">
          ...
          <tbody>
            {gimbalBanners?.length > 0 ? (
              gimbalBanners.map((item, index) => (
                <tr
                  key={item.id}
                ...
                >
                  ...
                  <td className="px-5 py-3 text-sm font-medium text-gray-800 max-w-xs truncate whitespace-pre-line">
                    {item.title}
                  </td>
                  ...
                  <td className="px-5 py-3">
                    <div className="flex justify-center gap-2">
                      <Link href={`/dashboard/gimbal-banner/edit-gimbal-banner/${item.id}`}>
                        <button
                      ...
                      <button
                        onClick={() => handleDeleteGimbalBanner(item)}
              ...
              <tr>
                <td colSpan={8} className="py-10 text-center text-gray-500">
                  No gimbal banner entries found.
                </td>
              </tr>
            )}
          </tbody>
...
export default AllGimbalBanner;
