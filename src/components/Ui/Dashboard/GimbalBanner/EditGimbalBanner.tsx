...
interface EditGimbalBannerProps {
  id: string;
}

interface EditGimbalBannerFormValues {
  title: string;
  eyebrow?: string;
  description?: string;
  button_text?: string;
...
const EditGimbalBanner = ({ id }: EditGimbalBannerProps) => {
  const router = useRouter();

  const [imagePreview, setImagePreview] = useState<string | null>(null);

  const { data: itemData, isLoading: isFetching } =
    useGetSingleGimbalBannerQuery(id);
  const [updateGimbalBanner, { isLoading: isUpdating }] =
    useUpdateGimbalBannerMutation();
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<EditGimbalBannerFormValues>();
  ...
  const onSubmit: SubmitHandler<EditGimbalBannerFormValues> = async (values) => {
    try {
      ...
      await updateGimbalBanner({ id, data: formData }).unwrap();
      toast.success("Gimbal banner updated successfully!");
      ...
      Swal.fire({
        ...
        text:
          (Array.isArray(error.data?.message)
            ...
            : error.data?.message) || "Failed to update gimbal banner.",
        icon: "error",
      });
    }
  };
    ...
    return (
      <div className="flex h-64 items-center justify-center rounded-lg border bg-white p-6">
        <div className="flex items-center gap-2 text-gray-500">
          ...
          <span>Loading gimbal banner details...</span>
        </div>
      </div>
    );
  }
    ...
    <div className="rounded-lg border bg-white border-gray-200 overflow-hidden p-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-8 space-y-6">
        ...
        <div className="flex justify-end gap-4 mt-6">
          ...
          <GradientButton
            type="submit"
            text={isUpdating ? "Updating..." : "Update Gimbal Banner"}
            icon={Save}
            disabled={isUpdating}
          />
        </div>
...
export default EditGimbalBanner;
