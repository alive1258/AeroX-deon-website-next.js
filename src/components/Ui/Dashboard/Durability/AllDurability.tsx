...
import { DurabilityItem } from "@/src/types/durabilityType";
...
const AllDurability: React.FC = () => {
  const [searchValue, setSearchValue] = useState("");
  const [currentPage, setCurrentPage] = useState(1);

  const debouncedSearch = useDebounce(searchValue, 500);

  const { data, isLoading, isFetching, refetch } = useGetAllDurabilityQuery({
    search: (debouncedSearch as string) || undefined,
    page: currentPage,
    limit: LIMIT,
  });

  const [deleteDurability] = useDeleteDurabilityMutation();

  const items: DurabilityItem[] = data?.data || [];
  ...
  const handleDeleteDurability = async (item: DurabilityItem) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Delete durability section "${item.title}"?`,
        icon: "warning",
      ...
      await deleteDurability(item.id).unwrap();

      await Swal.fire({
        icon: "success",
        title: "Deleted!",
        text: `Durability section "${item.title}" has been deleted.`,
        timer: 1000,
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 p-6 border-b border-gray-200">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Durability Section</h1>
          <p className="text-sm text-gray-500">
            Manage the homepage durability/specs content
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
          <input
            type="text"
            placeholder="Search durability entries..."
            value={searchValue}
          ...
          <Link href="/dashboard/durability/add-durability">
                ...
                >
                  ...
                  <td className="px-5 py-3">
                    <div className="flex justify-center gap-2">
                      <Link href={`/dashboard/durability/edit-durability/${item.id}`}>
                        <button
                      ...
                      <button
                        onClick={() => handleDeleteDurability(item)}
