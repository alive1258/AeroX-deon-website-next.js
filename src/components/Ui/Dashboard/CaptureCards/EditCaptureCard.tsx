...
} from "@/src/redux/api/captureCardApi";

interface EditCaptureCardProps {
  id: string;
}

interface EditCaptureCardFormValues {
  title: string;
  description?: string;
  button_link?: string;
  position?: number;
...
const ALL_CAPTURE_CARD_PATH = "/dashboard/capture-cards/all-capture-cards";

const EditCaptureCard = ({ id }: EditCaptureCardProps) => {
  const router = useRouter();

  const [imagePreview, setImagePreview] = useState<string | null>(null);

  const { data: cardData, isLoading: isFetching } = useGetSingleCaptureCardQuery(id);
  const [updateCaptureCard, { isLoading: isUpdating }] = useUpdateCaptureCardMutation();
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<EditCaptureCardFormValues>();
  ...
  const onSubmit: SubmitHandler<EditCaptureCardFormValues> = async (values) => {
    try {
      ...
      await updateCaptureCard({ id, data: formData }).unwrap();
      toast.success("Capture card updated successfully!");
      router.push(ALL_CAPTURE_CARD_PATH);
    } catch (err) {
      ...
      Swal.fire({
        ...
        text:
          (Array.isArray(error.data?.message)
            ...
            : error.data?.message) || "Failed to update capture card.",
        icon: "error",
      });
    }
  };
    ...
    return (
      <div className="flex h-64 items-center justify-center rounded-lg border bg-white p-6">
        <div className="flex items-center gap-2 text-gray-500">
          ...
          <span>Loading capture card details...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-lg border bg-white border-gray-200 overflow-hidden p-6">
      <PageHeader
        title="Edit Capture Card"
        breadcrumbs={[
          { title: "Dashboard", link: "/dashboard" },
          { title: "Capture Cards", link: ALL_CAPTURE_CARD_PATH },
          { title: "Edit Capture Card" },
        ]}
      />
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-8 space-y-6">
        ...
        <div className="flex justify-end gap-4 mt-6">
          ...
          <GradientButton
            type="submit"
            text={isUpdating ? "Updating..." : "Update Capture Card"}
            icon={Save}
            disabled={isUpdating}
          />
        </div>
...
export default EditCaptureCard;
