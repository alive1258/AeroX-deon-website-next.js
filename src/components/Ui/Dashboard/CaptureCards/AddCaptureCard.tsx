...
import { useCreateCaptureCardMutation } from "@/src/redux/api/captureCardApi";

interface AddCaptureCardFormValues {
  title: string;
  description?: string;
  button_link?: string;
  position?: number;
...
const AddCaptureCard = () => {
  const router = useRouter();
  const [imagePreview, setImagePreview] = useState<string | null>(null);

  const [createCaptureCard, { isLoading }] = useCreateCaptureCardMutation();
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<AddCaptureCardFormValues>({
    defaultValues: {
      title: "",
  ...
  const onSubmit: SubmitHandler<AddCaptureCardFormValues> = async (values) => {
    try {
      ...
      await createCaptureCard(formData).unwrap();
      toast.success("Capture card created successfully!");
      reset();
      setImagePreview(null);
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-8 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Input
            label="Title"
            text="title"
            placeholder="4K Ultra HD Camera"
          ...
          <Textarea
            ...
            text="description"
            placeholder="Crisp, detailed visuals in every frame."
        ...
        <div className="flex justify-end gap-4 mt-6">
          ...
          <GradientButton
            type="submit"
            text={isLoading ? "Saving..." : "Create Capture Card"}
            icon={Plus}
            disabled={isLoading}
          />
        </div>
