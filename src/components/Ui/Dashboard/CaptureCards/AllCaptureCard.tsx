...
import { CaptureCardItem } from "@/src/types/captureCardType";
...
const AllCaptureCard: React.FC = () => {
  const [searchValue, setSearchValue] = useState("");
  const [currentPage, setCurrentPage] = useState(1);

  const debouncedSearch = useDebounce(searchValue, 500);

  const { data, isLoading, isFetching, refetch } = useGetAllCaptureCardQuery({
    search: (debouncedSearch as string) || undefined,
    page: currentPage,
    limit: LIMIT,
  });

  const [deleteCaptureCard] = useDeleteCaptureCardMutation();

  const cards: CaptureCardItem[] = data?.data || [];
  ...
  const handleDeleteCard = async (card: CaptureCardItem) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Delete capture card "${card.title}"?`,
        icon: "warning",
      ...
      await deleteCaptureCard(card.id).unwrap();

      await Swal.fire({
        icon: "success",
        title: "Deleted!",
        text: `Capture card "${card.title}" has been deleted.`,
        timer: 1000,
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 p-6 border-b border-gray-200">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Capture Cards</h1>
          <p className="text-sm text-gray-500">
            Manage the homepage capture showcase cards
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
          <input
            type="text"
            placeholder="Search capture cards..."
            value={searchValue}
          ...
          <Link href="/dashboard/capture-cards/add-capture-card">
                ...
                >
                  ...
                  <td className="px-5 py-3">
                    <div className="flex justify-center gap-2">
                      <Link href={`/dashboard/capture-cards/edit-capture-card/${card.id}`}>
                        <button
