...
import { ProductBundleItem } from "@/src/types/productBundleType";
...
const AllProductBundle: React.FC = () => {
  const [searchValue, setSearchValue] = useState("");
  const [currentPage, setCurrentPage] = useState(1);

  const debouncedSearch = useDebounce(searchValue, 500);

  const { data, isLoading, isFetching, refetch } = useGetAllProductBundleQuery({
    search: (debouncedSearch as string) || undefined,
    page: currentPage,
    limit: LIMIT,
  });

  const [deleteProductBundle] = useDeleteProductBundleMutation();

  const bundles: ProductBundleItem[] = data?.data || [];
  ...
  const handleDeleteBundle = async (bundle: ProductBundleItem) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Delete product bundle "${bundle.name}"?`,
        icon: "warning",
      ...
      await deleteProductBundle(bundle.id).unwrap();

      await Swal.fire({
        icon: "success",
        title: "Deleted!",
        text: `Product bundle "${bundle.name}" has been deleted.`,
        timer: 1000,
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 p-6 border-b border-gray-200">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Manage the shop page's preorder bundle tiers
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
          ...
          <Link href="/dashboard/product-bundles/add-product-bundle">
                ...
                >
                  ...
                  <td className="px-5 py-3">
                    <div className="flex justify-center gap-2">
                      <Link href={`/dashboard/product-bundles/edit-product-bundle/${bundle.id}`}>
                        <button
...
export default AllProductBundle;
