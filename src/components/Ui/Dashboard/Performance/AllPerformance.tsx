...
import { PerformanceItem } from "@/src/types/performanceType";
...
const AllPerformance: React.FC = () => {
  ...
  const { data, isLoading, isFetching, refetch } = useGetAllPerformanceQuery({
  ...
  const items: PerformanceItem[] = data?.data || [];
  ...
  const handleDelete = async (item: PerformanceItem) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Delete performance section "${item.title}"?`,
        icon: "warning",
      ...
      await deletePerformance(item.id).unwrap();

      await Swal.fire({
        icon: "success",
        title: "Deleted!",
        text: `Performance section "${item.title}" has been deleted.`,
        timer: 1000,
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 p-6 border-b border-gray-200">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Performance Section</h1>
          <p className="text-sm text-gray-500">
            Manage the homepage performance content
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
          <input
            type="text"
            placeholder="Search performance entries..."
            value={searchValue}
          ...
          <Link href="/dashboard/performance/add-performance">
                ...
                >
                  ...
                  <td className="px-5 py-3">
                    <div className="flex justify-center gap-2">
                      <Link href={`/dashboard/performance/edit-performance/${item.id}`}>
                        <button
